...
async def fetchFlightGuatemala():    
    try:
        response = await getFromBackend('flight/view/guatemala')
        print(response)
...
def viewFlightGuatemala():
    ...
    with ui.element('div').classes('!w-full !min-h-screen !bg-gray-50 !pt-20 !pb-16'):
        ...
        with ui.card().classes('!w-full !max-w-6xl !mx-auto !mt-8 !shadow-lg !rounded-lg !border !border-gray-200 !px-6'):
            ui.label('Listado de Vuelos Guatemala').classes('!text-2xl !font-bold !text-gray-800 !mb-4 text-center')
            ...
            async def cargarVuelos():
                data = await fetchFlightGuatemala()
